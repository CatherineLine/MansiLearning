...
class AppDatabase {
  ...
  Future<int> getCompletedRiddlesCount(int userId) async => Sqflite.firstIntValue(await (await database).rawQuery('SELECT COUNT(*) as count FROM user_progress WHERE user_id = ? AND source_context = "riddle" AND is_completed = 1', [userId])) ?? 0;

  Future<int> getUserTotalScore(int userId) async => Sqflite.firstIntValue(await (await database).rawQuery('SELECT SUM(score) as total FROM user_progress WHERE user_id = ?', [userId])) ?? 0;

  Future<pb.UserProgress?> getRiddleProgress(int userId, int riddleId) async {
    final maps = await (await database).query('user_progress', where: 'user_id = ? AND riddle_id = ?', whereArgs: [userId, riddleId]);
  ...
  Future<int> addTranslation(te.Translation translation) async => (await database).insert('translations', translation.toMap());

  Future<List<Map<String, dynamic>>> getTranslationHistory({DateTime? startDate, DateTime? endDate, String? searchQuery}) async {
    final db = await database;
    return await db.query('translations', orderBy: 'id DESC', limit: 100);
  }

  Future<void> clearTranslationHistory() async => (await database).delete('translations');
  Future<void> removeDuplicateTranslations() async => (await database).rawDelete('''DELETE FROM translations WHERE id NOT IN (SELECT MAX(id) FROM translations GROUP BY source_text, target_text)''');
...
class Riddle {
  final int? id; final String questionText; final String answerText; final String? hintText; final String? difficultyLevel; final String? category;
