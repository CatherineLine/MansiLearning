...
class TheoryPage extends StatefulWidget {
  ...
  const TheoryPage({super.key, required this.moduleId, required this.level, required this.moduleTitle});
...
class _TheoryPageState extends State<TheoryPage> {
  ...
  void initState() {
    ...
    _tasksFuture = AppDatabase.instance.getTasks(widget.level);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      key: _scaffoldKey,
      appBar: AppBar(title: Text('${widget.moduleTitle} - Уровень ${widget.level}', style: const TextStyle(color: Colors.white)), backgroundColor: const Color(0xFF0A4B47)),
      body: FutureBuilder<List<Map<String, dynamic>>>(
        future: _theoryFuture,
        builder: (context, theorySnapshot) {
          if (theorySnapshot.connectionState == ConnectionState.waiting) return const Center(child: CircularProgressIndicator());
          return Column(children: [
            Expanded(child: SingleChildScrollView(padding: const EdgeInsets.all(16),
                child: theorySnapshot.hasData && theorySnapshot.data!.isNotEmpty
                    ? Text(theorySnapshot.data!.first['content_html'] ?? '', style: const TextStyle(fontSize: 18))
                    : const Text('Теория не требуется для этого уровня', style: TextStyle(fontSize: 18)))),
            Padding(padding: const EdgeInsets.all(16),
              child: Center(
                child: FutureBuilder<List<Map<String, dynamic>>>(
                  future: _tasksFuture,
                  builder: (context, tasksSnapshot) {
                    if (tasksSnapshot.connectionState == ConnectionState.waiting) return const CircularProgressIndicator();
                    final hasTasks = tasksSnapshot.hasData && tasksSnapshot.data!.isNotEmpty;
                    return ElevatedButton(
                      onPressed: hasTasks ? () {
                        Navigator.push(context, MaterialPageRoute(builder: (context) => TaskPage(
                            moduleId: widget.moduleId, level: widget.level, moduleTitle: widget.moduleTitle,
                            tasks: tasksSnapshot.data!, initialScore: 0)));
                      } : null,
  ...
  Widget _buildDrawer(BuildContext context) {
    return Drawer(child: Container(padding: const EdgeInsets.only(top: 40), decoration: const BoxDecoration(color: Color(0xFFE7E4DF)),
